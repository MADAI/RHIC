...
CWaveFunction_ppiplus_sqwell::CWaveFunction_ppiplus_sqwell(string parsfilename) : CWaveFunction(){
...
CWaveFunction_ppiplus_sqwell::~CWaveFunction_ppiplus_sqwell(){
  SquareWell_DeleteArrays();
}

double CWaveFunction_ppiplus_sqwell::CalcPsiSquared(int iq,double r,double ctheta){
  double psisquared,x,theta=acos(ctheta);
  double q=GetQ(iq);
  complex<double> psi,DelPhi[3];
