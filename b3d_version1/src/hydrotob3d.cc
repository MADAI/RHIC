...
void CHYDROtoB3D::Init3D(){
	...
	//printf("epsilon_H=%g\n",intrinsic->epsilon);
	density=new double[nres];
	ring3d=new CRing*[NETA];
	nrings3d=new int[NETA];
	for(ieta=0;ieta<NETA;ieta++){
		ring3d[ieta]=new CRing[b3d->NRINGSMAX+1];
		nrings3d[ieta]=0;
	}
	epsilon=P=0.0;
	CResInfo *resinfo=reslist->GfirstResInfoptr;
...
int CHYDROtoB3D::MakeEvent(){
	...
	printf("before b3d, meanpt for pions=%g, meanu*phat=%g\n",meanpt/double(normpt),meanu/double(normpt));
					...
					if(pdotV>0.0){
						...
						if(randy->ran()<weight){
							...
							if(ID==111 || ID==211 || ID==-211){
								...
								//meanu+=(ptilde[1]*u[1]+ptilde[2]*u[2])/sqrt(ptilde[1]*ptilde[1]+ptilde[2]*ptilde[2]);
								meanu+=(p[1]*u[1]+p[2]*u[2])/pt;
								//meanu+=sqrt(u[1]*u[1]+u[2]*u[2]);
								normpt+=1;
							}
...
int CHYDROtoB3D::GenerateParticles3D(int ieta,int iring){
	CRing *earlier=&ring3d[ieta][iring-1];
	CRing *later=&ring3d[ieta][iring];
	//printf("iring1=%d, tau1=%g, iring2=%d, tau2=%g\n",iring-1,earlier->tau,iring,later->tau);
	CResInfo *resinfo;
...
CRing::CRing(){
	...
	dToverH=new double**[nphi+1];
	for(iphi=0;iphi<=nphi;iphi++){
		dToverH[iphi]=new double*[4];
		for(alpha=0;alpha<4;alpha++){
			dToverH[iphi][alpha]=new double[4];
...
void CRing::Read(FILE *fptr){
	...
	double dToverH_xx[300],dToverH_xy[300],dToverH_yy[300];
...
void CRing::FillOutArrays(double *x,double *y,double *uxi,double *uyi,double *dToverH_xx,double *dToverH_xy,double *dToverH_yy){
	//char filename[120];
	//FILE *ringinfo;
